<?php session_start() ?>
<!-- Read skaito info ir ja atvaziduoja, metodas GET -->
<!DOCTYPE html>
<html lang="en">
...
<body>
        ...
        <div class="row">
            <div class="col-2">
                <form action="" method="get">
                    ...
                    <a href="http://localhost/backEnd/php/crud/read.php" class="btn btn-secondary">Clear</a>
                </form>
            </div>
        </div>
    </div>
    ...
    <ul class="list-group list-group-flush">
        ...
        <!-- iterpiam jei sortas per GET, tuomet -->
        <?php
        if (isset($_GET['sort'])) {
            match ($_GET['sort']) {
                'amount_asc' => usort($boxes, fn ($a, $b) => $a['amount'] <=> $b['amount']),
                'amount_desc' => usort($boxes, fn ($a, $b) => $b['amount'] <=> $a['amount']),
                'id_asc' => usort($boxes, fn ($a, $b) => $b['boxId'] <=> $a['boxId']),
                'id_desc' => usort($boxes, fn ($a, $b) => $a['boxId'] <=> $b['boxId']),
                default => $boxes,
            };
        }
        ?>
